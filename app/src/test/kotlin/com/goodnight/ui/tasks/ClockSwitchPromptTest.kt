...
class ClockSwitchPromptTest {
    ...
    @Test fun idleTakesEffectDirectly() {
        assertEquals(ClockPickAction.FREE, clockPickAction(null, 7L))
        assertEquals(
            "IDLE 快照(防御)同样直接生效",
            ClockPickAction.FREE,
            clockPickAction(snap(EngineStatus.IDLE, 7L), 9L),
        )
    }

    @Test fun pickingTheRunningClockIsANoOp() {
        assertEquals(ClockPickAction.SAME_CLOCK, clockPickAction(snap(EngineStatus.RUNNING, 7L), 7L))
    }

    @Test fun pickingAnotherClockWhileRunningAsksFirst() {
        assertEquals(ClockPickAction.ASK_CONFIRM, clockPickAction(snap(EngineStatus.RUNNING, 7L), 8L))
    }

    /** 暂停中也是「有会话」:换时钟同样先确认(设计 §4:不允许段内静默换规则) */
    @Test fun pausedSessionAlsoAsksFirst() {
        assertEquals(ClockPickAction.ASK_CONFIRM, clockPickAction(snap(EngineStatus.PAUSED, 7L), 8L))
        assertEquals(ClockPickAction.SAME_CLOCK, clockPickAction(snap(EngineStatus.PAUSED, 7L), 7L))
    }
