...
import com.goodnight.service.TimerNotifications
import kotlinx.coroutines.flow.debounce
import kotlinx.coroutines.launch

/** open:测试用空 onCreate 子类注入受控 AppGraph(Robolectric 绕过真实装配) */
open class GoodNightApp : Application() {
    lateinit var graph: AppGraph

    override fun onCreate() {
        super.onCreate()
        graph = AppGraph(this)
        com.goodnight.diag.DiagLog.markEnabled(this)
        com.goodnight.diag.DiagLog.add("App", "onCreate（进程启动）")
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            override fun onActivityStarted(a: android.app.Activity) {
                com.goodnight.diag.DiagState.appForeground = true
                com.goodnight.diag.DiagLog.add("App", "进入前台")
            }
            override fun onActivityStopped(a: android.app.Activity) {
                com.goodnight.diag.DiagState.appForeground = false
                com.goodnight.diag.DiagLog.add("App", "退到后台")
            }
        ...
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Default).launch {
            runCatching { TimerNotifications.ensureChannels(this@GoodNightApp) }
            // v1.1 #5:报表通知闹钟(周日/月末 23:00)——每次进程冷启/开机补武装(闹钟不跨重启)
            runCatching { com.goodnight.service.ReportAlarmScheduler(this@GoodNightApp).ensure() }
            // v1.13.0:备份目录授权自愈 —— 修复"重启后备份目录失效"(选目录时未持久化授权)
            runCatching { com.goodnight.data.BackupPermissions.healOnStartup(this@GoodNightApp, graph.settingsRepo) }
        }
        ...
        if (!prefs.getBoolean("normalized_v1111", false)) {
            kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Default).launch {
                ...
                val ok = runCatching {
                    com.goodnight.data.SessionNormalizer.normalizeAllSessionsOnce(
                        graph.db, graph.db.focusSessionDao(), graph.totalsRepo,
                    )
                }.onFailure { android.util.Log.w("GoodNightApp", "session normalize failed", it) }.isSuccess
        ...
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Default).launch {
            runCatching {
                graph.totalsRepo.dataTick()
                    ...
                    .collect { com.goodnight.data.AutoBackupScheduler.scheduleNow(this@GoodNightApp) }
            }
        }
    }
