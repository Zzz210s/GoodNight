...
- 时钟按任务分组:通用段 + 各任务专属段,从任务卡片新建的时钟落在该任务段。点任务卡片上的时钟 chip 即开始并绑定该任务,计时卡显示「任务 · 时钟」,计时中换时钟先弹确认。删有历史的时钟改为归档(名字与时长仍在报表与每日详情),删任务则把专属时钟还给通用段。备份格式升到 v3(兼容 v1/v2)。

## 安装

要求:Android 8.0(API 26)及以上。

> **v2.0.0 变更包名**(`com.embertimer` → `com.goodnight`):它是一个**新的应用身份**,旧版不能覆盖升级。
> 升级步骤:先在旧版里做一次备份(设置 → 数据备份 → 备份)→ 卸载旧版 → 安装 2.0.0 → 在设置里恢复备份。
